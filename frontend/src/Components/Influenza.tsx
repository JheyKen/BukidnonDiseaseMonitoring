...
function Influenza() {
  ...
  const handleVictimsCount = async () => {
    try {
      ...
      while (x < Municipality.length) {
        ...
        const result: AxiosResponse = await Service.getVictimsCountPerMunicipality("influenza", municipalityData.name);
        const { data } = result
  ...
  const handleVictimsGender = async () => {
    try {
      const result: AxiosResponse = await Service.getVictimsCountPerGender("influenza")
      const { data } = result
  ...
  const handleVictimsAge = async () => {
    try {
      const result: AxiosResponse = await Service.getVictimsCountPerAge("influenza")
      const { data } = result
      setVictimsAge(data)
    } catch (error) {
      alert("Error fetching record.")
