...
function Influenza() {
  ...
  const handleVictimsGender = async () => {
    try {
      ...
      const result: AxiosResponse = await Service.getVictimsCountPerGender("influenza", date_from, date_to)
      const { data } = result
  ...
  const handleVictimsAge = async () => {
    try {
      ...
      const result: AxiosResponse = await Service.getVictimsCountPerAge("influenza", date_from, date_to)
      const { data } = result
      setVictimsAge(data)
    } catch (error) {
      alert("Error fetching record.")
