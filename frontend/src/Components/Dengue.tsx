...
function Dengue() {
  ...
  const handleVictimsCount = async () => {
    try {
      ...
      while (x < Municipality.length) {
        ...
        const caseResult: AxiosResponse = await Service.getVictimsCountPerMunicipality("dengue", municipalityData.name, date_from, date_to);
        const { data: cases } = caseResult

        const deathResult: AxiosResponse = await Service.getDeathCountPerMunicipality("dengue", municipalityData.name, date_from, date_to);
  ...
  const handleVictimsGender = async () => {
    try {
      ...
      const result: AxiosResponse = await Service.getVictimsCountPerGender("dengue", date_from, date_to)
      const { data } = result
  ...
  const handleVictimsAge = async () => {
    try {
      ...
      const result: AxiosResponse = await Service.getVictimsCountPerAge("dengue", date_from, date_to)
      const { data } = result
      setVictimsAge(data)
    } catch (error) {
      alert("Error fetching record.")
          ...
          <Paper elevation={5} className='report-dengue-bar-paper'>
            <table>
              <tr>
                ...
                <td>
                  <TextField
                    id="disease-date-field"
                    name="dateFrom"
                    type="date"
                    value={dateFrom}
                    onChange={(event: ChangeEvent<HTMLInputElement>) => handleDateFrom(event)}
                    fullWidth
                    required />
                </td>
                <td>
                ...
                <td>
                  <TextField
                    ...
                    type="date"
                    value={dateTo}
                    onChange={(event: ChangeEvent<HTMLInputElement>) => handleDateTo(event)}
                    fullWidth
                    required />
                </td>
              </tr>
